...
class Game extends UI {
  ...
  #newGame(rows = this.#config.easy.rows, cols = this.#config.easy.cols, mines = this.#config.easy.mines) {
    ...
    this.#buttons.reset.chengeEmotion("neutral");
    ...
    if (!isWin) {
      ...
      this.#buttons.reset.chengeEmotion("negative");
      this.#modal.setText();
    ...
    this.#buttons.reset.chengeEmotion("positive");
    this.#modal.setText();
    this.#modal.toggleModal();
  }
  ...
  #setCellValue(cell) {
    ...
    for (let rowIndex = Math.max(cell.y - 1, 0); rowIndex <= Math.min(cell.y + 1, this.#numberOfRows - 1); rowIndex++) {
      for (
        let collIndex = Math.max(cell.x - 1, 0);
        collIndex <= Math.min(cell.x + 1, this.#numberOfCols - 1);
        collIndex++
      ) {
        if (this.#cells[rowIndex][collIndex].isMine) minesCount++;
      }
    }
